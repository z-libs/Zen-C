...
tre_stack_t *tre_stack_new(size_t size, size_t max_size)
{
    tre_stack_t *s;

    s = (decltype(s))xmalloc(sizeof(*s));
    if (s != NULL)
    {
        s->stack = (decltype(s->stack))xmalloc(sizeof(*s->stack) * size);
        if (s->stack == NULL)
        {
            xfree(s);
            return NULL;
...
size_t tre_stack_num_items(tre_stack_t *s)
{
    return s->ptr;
}
...
static reg_errcode_t tre_stack_push(tre_stack_t *s, union tre_stack_item value)
{
    ...
    else
    {
        ...
        else
        {
            ...
            new_buffer = (decltype(new_buffer))xrealloc(s->stack, sizeof(*new_buffer) * new_size);
            if (new_buffer == NULL)
            {
