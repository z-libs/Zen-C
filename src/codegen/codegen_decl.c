...
void emit_preamble(ParserContext *ctx, FILE *out)
{
    ...
    else
    {
        ...
        fputs("    fflush(stdout);\n", out);
        fputs("    if (_z_orig_stdout == -1) _z_orig_stdout = zc_dup(ZC_STDOUT_FILENO);\n", out);
        fputs("    int nullfd = zc_open(\"/dev/null\", ZC_O_WRONLY);\n", out);
        fputs("    zc_dup2(nullfd, ZC_STDOUT_FILENO);\n", out);
        fputs("    zc_close(nullfd);\n", out);
        fputs("}\n", out);
        fputs("void _z_restore_stdout() {\n", out);
        fputs("    fflush(stdout);\n", out);
        fputs("    if (_z_orig_stdout != -1) {\n", out);
        fputs("        zc_dup2(_z_orig_stdout, ZC_STDOUT_FILENO);\n", out);
        fputs("        zc_close(_z_orig_stdout);\n", out);
